...
public class SqlRepository implements Repository {
    ...
    */
    ...
    private static final String CATEGORY_DELETE_ASSIGNMENTS = "{ CALL deleteAllAssignmentsOfCategory (?) }";
    ...
    private static final String CATEGORY_SEARCH_SINGLE_BY_NAME = "{ CALL searchCategoryByName (?) }";
    ...
    private static final String ARTICLE_UNASSIGN_CATEGORY = "{ CALL deleteCategoryAssignments (?,?) }";
    ...
    public Optional<Category> getCategoryByName(String name) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection conn = dataSource.getConnection();
                CallableStatement stmt = conn.prepareCall(CATEGORY_SEARCH_SINGLE_BY_NAME)) {
            stmt.setString(1, name);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return Optional.of(new Category(
    ...
    @Override
    public void removeCategoryAssignments(int id) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection();
                CallableStatement stmt = con.prepareCall(CATEGORY_DELETE_ASSIGNMENTS)) {
            stmt.setInt(1, id);
            stmt.executeUpdate();
        }
    }
    ...
    @Override
    public void assignCategoryToArticle(int categoryId, int articleId) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection();
    ...
    @Override
    public void unassignCategoryFromArticle(int categoryId, int articleId) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection();
                CallableStatement stmt = con.prepareCall(ARTICLE_UNASSIGN_CATEGORY)) {
            stmt.setInt(1, categoryId);
            stmt.setInt(2, articleId);
            stmt.executeUpdate();
        }
